...
class PlayerWithBluffStrategy(Player):
    ...
    def step(self):
        ...
        time_reveal = self.time_estimate - self.time_reveal_epsilon - self.model.global_delay - self.delay
        if self.model.schedule.time < time_reveal:
            self.bid = self.model.public_signal_value + self.bluff_factor * self.aggregated_signal - self.pm
...
class Auction(Model):
    def __init__(self, N, A, L, S, B, rate_public_mean, rate_public_sd, rate_private_mean, rate_private_sd,
                 T_mean, T_sd, delay, custom=False):
    ...
    def manually_setup_agents(self):
        ...
        for strategy, count in items:
            for j in range(count):
                ...
                if j % 4 < 2:
                    ...
                    mean = np.exp(3.85 + (1.58**2) / 2)

                    # Sample until conditions are satisfied
                    while True:
                        delay = int(np.random.lognormal(mean=3.85, sigma=1.58))
                        if delay < mean:
                            break
