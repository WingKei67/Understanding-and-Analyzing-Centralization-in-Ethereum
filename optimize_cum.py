...
for index in range(8):
    ...
    grouped_cum = df_filtered.groupby("agent_group")["Profit"].sum()
    ...
    bar_colors = [color_map.get(agent, '#7f7f7f') for agent in grouped_cum.index]
    ...
    bars = plt.bar(grouped_cum.index, grouped_cum.values, color=bar_colors, edgecolor='black')
    ...
    plt.title(f"Cumulative Profit by Strategy {dimension_mapping[int(index)]}")
    ...
    # Save the figure
    filename = f"plot/cumulative_profit_by_agent_group_{index}.png"
