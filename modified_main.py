...
class Auction(Model):
    ...
    def step(self):
        ...
        for _ in range(new_public_signal) :
            signal_value = np.random.lognormal(mean=-4.902023, sigma=4.412283)
        ...
        for _ in range(new_private_signal) :
            private_signal_value = np.random.lognormal(mean=-7.054694, sigma=5.184989)
...
model = Auction(N=4, A=4, L=4, S=0, B=0, rate_public_mean=0.2014, rate_public_sd=0.1237, rate_private_mean=0.1158, rate_private_sd=0.0819, T_mean=12, T_sd=0, delay=1)
