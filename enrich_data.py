...
def open_file(FOLDER):
    ...
    for file in os.listdir(FOLDER):
        if not (file.startswith(f"mevboost_{index}.") and file.endswith(".csv")):
            continue
...
def open_enriched_file(FOLDER):
    df = pd.DataFrame()
    for file in os.listdir(FOLDER):
        if file != f"mevboost_e{index}.csv":
            continue
        print("File found")
...
def enrich_data(w3, df, df_txs):
    ...
    try:
        ...
        df = df[df["miner"] != "none"]
        df.to_csv(FOLDER + f"mevboost_e{index}.csv", index=None)
        df_txs.to_csv(FOLDER + f"mevboost_e_txs_{counter_txs}.csv", index=None)
        counter_txs += 1
        print(f"\nStoring successful")
